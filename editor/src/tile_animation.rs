...
impl GroundAnimationTable {
    /// Parses `gndani.tbl` bytes.
    ///
    /// The reference tools store ground animation tile IDs as raw tileset
    /// indexes. The editor stores ground IDs as 1-based values with `0`
    /// reserved for "empty", so we normalize every parsed tile ID by `+1`.
    pub fn from_gndani_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        for line in text.lines() {
            ...
            let tile_sequence: Vec<u16> =
                tile_ids.iter().map(|tile_id| tile_id.saturating_add(1)).collect();
    ...
    pub fn minimum_interval_ms(&self) -> Option<u32> {
        self.entries.iter().map(|entry| entry.animation_interval_ms).min()
    }
    ...
    pub fn animated_tile_id(&self, tile_id: u16, time_seconds: f64) -> u16 {
        ...
        let frame_offset =
            (elapsed_ms / u64::from(entry.animation_interval_ms)) % entry.tile_sequence.len() as u64;
...
mod tests {
    ...
    fn rotates_from_current_tile() {
        ...
        assert_eq!(table.animated_tile_id(2, 0.0), 2);
        assert_eq!(table.animated_tile_id(2, 0.5), 3);
        assert_eq!(table.animated_tile_id(2, 1.0), 1);
    }
